SimpleLogTrendRegression <- function(data, 
  ...
  plot(timestamps, log(data), lty=1, type="l",
       ...
       ylab=paste("Log", data.identifier))
  ...
  plot(timestamps, data, lty=1, type="l",
       main = plot.txt.lvl,
       xlim = c(first.date, tail(timestamps.oos, 1)), 
       xlab="Time", ylab="data.identifier",
  ...
  result.log.trend        <- xts(as.matrix(model.pred.oos, ncol=1), 
                                 order.by = timestamps.oos)
  names(result.log.trend) <- "log.trend"
  result.log.sd1          <- model.sd
  
  result.level.trend        <- xts(as.matrix(exp(model.pred.oos), ncol=1),
                                   order.by = timestamps.oos)
  names(result.level.trend) <- "level.trend"
  
  return( list(log.trend    = result.log.trend,
               log.trend.sd = result.log.sd1,
               level.trend  = result.level.trend) )
}